...
public abstract class AbstractConnection implements Connection {
	...
	public AbstractConnection() {
		internalQueue = Collections
				.synchronizedList(new LinkedList<KtnDatagram>());
		externalQueue = Collections
				.synchronizedList(new LinkedList<KtnDatagram>());
		isReceiving = false;
	 ...
	 */
	protected synchronized void simplySendPacket(KtnDatagram packet)
			throws ClException, IOException {
	...
	protected synchronized KtnDatagram sendDataPacketWithRetransmit(
		...
		timer.scheduleAtFixedRate(new SendTimer(new ClSocket(), packet), 0,
				RETRANSMIT);
	 ...
	 * <br>
	 * This method relies on {@link #constructInternalPacket(Flag)}, i.e.
	 * myAddress, myPort, remoteAddress, remotePort and sequenceNo must be
	...
	protected synchronized void sendAck(KtnDatagram packetToAck, boolean synAck)
		...
		KtnDatagram ackToSend = constructInternalPacket(synAck ? Flag.SYN_ACK
				: Flag.ACK);
		ackToSend.setAck(packetToAck.getSeq_nr());

		// Send the ack, trying at most `tries' times.
		Log.writeToLog(ackToSend, "Sending Ack: " + ackToSend.getAck(),
				"AbstractConnection");
	 ...
	 *             state.
	 */
	protected KtnDatagram receivePacket(boolean internal) throws IOException,
			EOFException {
		/*
			...
			while (System.currentTimeMillis() - start < TIMEOUT) {
				...
				if (incomingPacket == null) {
					...
					if (incomingPacket.getFlag() != Flag.NONE) {
						// Packet is internal
						Log.writeToLog(
								"Received an internal packet in doReceive",
								"AbstractConnection");

						if (incomingPacket.getFlag() == Flag.FIN
						...
						Log.writeToLog(
								"Received an external packet in doReceive",
								"AbstractConnection");
	...
	protected KtnDatagram receiveAck() throws IOException, EOFException {
		...
		Log.writeToLog("Waiting for incoming packet in receiveAck()",
				"AbstractConnection");
		...
		while (System.currentTimeMillis() - start < TIMEOUT) {
			...
			// die, but never less than 1 millisecond
			try {
				receiver.join(Math.max(TIMEOUT
